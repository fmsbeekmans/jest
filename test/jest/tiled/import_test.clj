...
(cell/with-initialized-temp-world [2 2]
  (let [c world/cell]
    ...
    (fact "place-building should place a truck spawner in case of valid buildings"
            (do
              (#'import/place-building (c [0 0]) :spawn-truck)
              (let [t (c [0 0])]
                (:building-type t) => :spawn
                (:vehicle-type t) => :truck)))
    (fact "place-paths should construct paths in both the current and adjacent cell"
          (let [from #(c [0 0])
                d :east
...
(fact "initialize world should create a world with the correct properties"
...
(cell/with-initialized-temp-world [2 2]
  (fact "parser should correctly dispatch to background mm"
        (let [l {:name :background :data [..bg1.. ..bg2.. ..bg3.. ..bg4..]}
              cells (#'import/workable-world)
