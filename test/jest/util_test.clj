...
(fact "maybe-deref should behave as the identity function for non-derefables"
...
(fact "ncomp with n = 0 should behave like the identity function"
...
(fact "ncomp should be able to compose incrementors and decrementors"
...
(fact "offset-vec should return an explicitly indexed version of vec when offset = 0"
      (util/offset-vec [:0 :1 :2 :3] 0) => {0 :0
                                            1 :1
                                            2 :2
                                            3 :3})

(fact "offset-vec should return an offsetted explicitly indexed version of vec"
      (util/offset-vec [:0 :1 :2 :3] 4) => {4 :0
                                            5 :1
                                            6 :2
                                            7 :3})

(fact "offset-map should return the empty map if the input is also an empty map"
      (util/offset-map {} 42) => {})
...
(fact "two-step-map should return an empty map if either of the input maps is empty"
      (util/two-step-map {} {}) => {}
      (util/two-step-map {:a :a :b :b} {}) => {}
      (util/two-step-map {} {:a :a :b :b} ) => {})

(fact "two step map should contain linked entries"
      (util/two-step-map {:a :1 :b :2 :c :3} {:1 :a :23 :b :c :3}) => {:a :a}
