...
(fact "@game-time returns the value returned by (calculate-game-time)"
      @s/game-time => ..game-time..
      (provided (s/calculate-game-time) => ..game-time..))

(scheduler-fact "A fact that is scheduled is run"
                (s/start!)
                (let [returned (promise)]
                  (s/schedule #(deliver returned :done) 0)
                  @returned => :done))

(scheduler-fact "facts that are scheduled while paused are run after resuming."
                (s/start!)
                (s/pause!)
...
(fact "offset calculates right offsets"
      (s/offset 1) => 9001
