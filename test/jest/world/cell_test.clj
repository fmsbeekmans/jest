...
(cell/with-initialized-temp-world [10 10]
  ...
  (fact "The borders of the map are unconnected on their border edge."
        (doall (for [x '(0) y (range 10)]
                 (world/direction-exists? (world/cell [x y]) :west))) => (n-of false 10)

        (doall (for [x '(9) y (range 10)]
                 (world/direction-exists? (world/cell [x y]) :east))) => (n-of false 10)

        (doall (for [x (range 10) y '(0)]
                 (world/direction-exists? (world/cell [x y]) :north))) => (n-of false 10)

        (doall (for [x (range 10) y '(9)]
                 (world/direction-exists? (world/cell [x y]) :south))) => (n-of false 10))
